...
namespace UnityEditor.TimelineEditor
{
    public interface ITimeline
    {
        ...
        float magnetSnapInterval { get; }
        List<TimelineWindowBase.GuideLine> pendingGuides { get; set; }
        float maxTime { get; }
        bool anyClipDragging{ get; set; }
        void OnClipDoubleClick(Rect rect, Clip action);
        void SafeDoAction(System.Action call);
        void InitClipWrappers();
        List<ClipWrapper> multiSelection { get; set; }
    }

    public class TimelineWindowBase : EditorWindow, ITimeline
    {
        ...
        [HideInInspector]
        private List<GuideLine> _pendingGuides = new List<GuideLine>();
        public List<GuideLine> pendingGuides
        {
        ...
        public static TimelineWindowBase current { get; set; }
        ...
        void OnGUI()
        {
            ...
            if (cutscene == null)
            {
                ShowNotification(new GUIContent(SHOWINFO));
                return;
            }
            //...
            DoKeyboardShortcuts();
        ...
        void ShowListGroups(Event e, ref float nextYPos)
        {
            ...
            for (int g = 0; g < cutscene.groups.Count; g++)
            {
                ...
                {
                    var menu = new GenericMenu();
                    foreach (var _info in UnityEditor.TimelineEditor.EditorTools.GetTypeMetaDerivedFrom(CanAddTrackBaseType, Prefs.isOld))
                    {
                        var info = _info;
        ...
        void ShowToolbar()
        {
            ...
            {
                PreferencesWindow.Show(new Rect(screenWidth - 5 - 400, TOOLBAR_HEIGHT + 5, 400, screenHeight - TOOLBAR_HEIGHT - 50), OnPreferencesDraw);
            }
        ...
        protected virtual void OnShowToolbar()
        {
